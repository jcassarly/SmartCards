...
|Busy JSON Query|Tests that when sending the JSON query to the Pi, if the deck is locked due to flashing a card, the response will be a busy error|1.|results||
|Lock/Unlock Query|Tests that the App can lock the deck object on the Pi - inhibiting flashing and other queries until unlocked|1. |results||
|Busy Unlock Query|Tests that if the app sends an unlock query that it cannot unlock the Pi if the deck is locked due to flashing|1. |results||
|Override Query|Tests that the App can send a new decklist to the Pi, overriding the preexisting decklist|1. |results||
|Override Blocks Flashing|Tests that the Pi cannot flash more cards until the override is done sending images|1. |Results||
